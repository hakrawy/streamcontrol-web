...
import { View, Text, Pressable, TextInput, StyleSheet, ActivityIndicator, useWindowDimensions } from 'react-native';
...
export default function SearchScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      try {
        setResults(await api.searchCatalog(query.trim()));
      } catch {
        setResults([]);
      } finally {
        setSearching(false);
    ...
    if (activeGenre) {
      items = items.filter((item: any) => {
        if (item.type === 'channel') {
          return String(item.category || '').toLowerCase() === activeGenre.toLowerCase();
        }
  ...
  const subtitle = (item: api.SearchResultItem) =>
    item.type === 'channel' ? String(item.category || copy.live).toUpperCase() : (item.genre?.[0] || copy.featured);
      ...
      <SafeAreaView edges={['top']} style={[styles.container, { direction }]}>
        <View style={styles.headerCard}>
          <View style={styles.headerRow}>
            <View>
              <Text style={styles.eyebrow}>{copy.results}</Text>
              <Text style={styles.pageTitle}>{copy.results}</Text>
            </View>
            <View style={styles.countPill}>
              <MaterialIcons name="grid-view" size={14} color="#D7F0FF" />
              <Text style={styles.countPillText}>{visibleItems.length}</Text>
            </View>
          </View>
          ...
          {searching ? (
            ...
            <FlashList
              ...
              renderItem={({ item, index }) => (
                <Animated.View
                  entering={FadeInDown.delay(Math.min(index, 10) * 28).duration(260)}
                  style={{ flex: 1, marginBottom: 12, paddingRight: index % gridColumns === gridColumns - 1 ? 0 : 12 }}
                >
                  <Pressable onPress={() => openItem(item)}>
                    <View style={styles.card}>
                      <View style={styles.posterWrap}>
                        <Image
                          source={{ uri: posterUri(item) }}
                          style={styles.poster}
                          contentFit={item.type === 'channel' ? 'contain' : 'cover'}
                          transition={perf.imageTransition}
                        />
                        {item.type !== 'channel' && (item as any).is_new ? (
                        ...
                        <View style={styles.metaRow}>
                          {item.type !== 'channel' ? (
                            <>
                              ...
                              <Text style={styles.rating}>{(item as any).rating}</Text>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { fontSize: 11, fontWeight: '800', color: '#8ED8FF', letterSpacing: 1.4, marginBottom: 4 },
  pageTitle: { fontSize: 24, fontWeight: '800', color: '#FFF', letterSpacing: -0.4 },
  countPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  countPillText: { fontSize: 12, fontWeight: '800', color: '#D7F0FF' },
  searchBar: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
  ...
  card: { backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 20, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)' },
  posterWrap: { width: '100%', aspectRatio: 2 / 3, backgroundColor: 'rgba(255,255,255,0.04)' },
  poster: { width: '100%', height: '100%' },
  newBadge: { position: 'absolute', top: 6, left: 6, backgroundColor: theme.primary, paddingHorizontal: 6, paddingVertical: 3, borderRadius: 999 },
